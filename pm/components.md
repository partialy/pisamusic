...
| 启动页多操作面板 | 启动断网重试、版本更新下载与跳转 | `pm/app/src/main/java/cn/partialy/pm/ui/dialog/SplashActionBottomSheet.kt` | `layout_splash_action_bottom_sheet.xml`、`bg_splash_content_card.xml`、`bg_bottom_radius_sheet.xml` | 参考公告 Sheet 结构，包含主副标题、更新说明卡片及动态排布按钮。按钮数量为 2 时水平 1:1 排列，>=3 时垂直全宽排列；主按钮使用品牌主色并在下载时动态显示进度与防重禁用，次按钮使用中性灰色或弱化底色。 |
...
| 主界面侧拉栏账号头与功能卡 | 系统账号、KG/WY 第三方账号、设置/扫码、导入歌单和定时关闭入口 | `pm/app/src/main/res/layout/main_drawer_content.xml` | `MainActivity`、`AccountSessionStore`、`MusicCookieManager`、`SongSourceTagBinder`、`SleepTimerManager` | 顶部账号头只展示 PisaMusic 系统账号，右箭头紧跟单行昵称，设置位于扫码左侧。功能卡首行为 KG/WY 左右双列，账号行额外使用 8dp 外层水平内边距，使左侧 K 与下方 16dp 功能图标基线对齐；未登录显示来源标签与“未登录 >”，点击进入各自登录，已登录显示来源标签、24dp 圆形头像和昵称。两个来源都未登录时导入行显示“登录后可导入歌单”，任一登录时显示“导入歌单”和红色“清除登录”；定时关闭启用后在行尾显示状态。 |
| 定时关闭 Sheet | 四档快捷时长、自定义时长、完整歌曲开关、剩余时间、更新与取消 | `bottom_sheet_sleep_timer.xml`、`bg_bottom_radius_sheet.xml`、`bg_lyric_settings_stepper.xml` | `SleepTimerBottomSheet`、`SleepTimerManager`、`SleepTimerRules`、`MusicController.setSongEndedInterceptor` | 复用通用 Material BottomSheet、`PmSwitch` 和歌词设置步进器样式；快捷区不显示标题，四档使用等距 60dp 圆形双行按钮，上方为数字、下方为 `min`。未选中为透明背景、中性色圆边框和文本，选中为淡 primary 背景、primary 边框和文本。默认四档为 5/15/30/60 分钟，可在“播放设置 → 定时配置”修改；“自定义”范围 1～1439 分钟。完整歌曲开关只显示标题，倒计时结束转入等待态，并在自然播放完成时同步拦截自动续播后暂停。 |
| 定时快捷配置 Sheet | 修改定时关闭的四个快捷档位 | `bottom_sheet_sleep_timer_presets.xml` | `SleepTimerPresetSettingsBottomSheet`、`PlaybackSettingsActivity`、`SleepTimerManager.presets` | 位于“设置 → 播放设置 → 定时配置”，四个 Material 数字输入分别对应 Sheet 的四个圆形按钮，范围均为 1～1439 分钟，保存后播放设置摘要立即刷新。 |
...
| 歌单详情推拉门折叠工具条 / 搜索栏 | 四类歌单共用大封面 Header、双按钮、顶栏三动作及 52dp 折叠工具条 | `pm/app/src/main/res/layout/activity_playlist_detail.xml`、`item_playlist_detail_header.xml`、`playlist_detail_sticky_play_all.xml` | `ui/playlistdetail/PlaylistDetailHeaderController`、`PlaylistDetailContentAdapter`、`PlaylistDetailInteractionController`、`ui/collapsing/CollapsingHeaderPolicy`、`bg_playlist_detail_hero_top_scrim.xml`、`bg_playlist_detail_hero_bottom_fade.xml` | 与“我的”页同构：全宽封面信息区位于 `CollapsingToolbarLayout` 的 parallax 层，顶部 Header 按相同 200dp 进度渐显，唯一 52dp 圆角工具条作为 AppBar 固定子项随内容上移，两者最终在封面前闭合；顶栏固定搜索/分享/更多（标题最多 1 行尾部省略），工具条左侧为播放图标 + 13sp 曲目数，右侧排序按钮打开默认/歌名 A-Z/歌手 A-Z 选项 Sheet。不得加入 RecyclerView Header、Anchor 手动位移或双工具条切换。 |
