...
- 在线歌曲播放失败不再自动跳到当前队列下一曲；失败后按设置里的“自动切换列表”处理，关闭时暂停并提示，开启时切换到本地 / 已缓存 / 已下载歌曲列表。在线播放缓存统一通过 `player/cache/PlaybackMediaCache` 门面访问，队列项必须先在 IO 完成 descriptor / catalog 登记，再以 `pmcache://media/<cacheKey>` 逻辑 URI 一次性交付播放器，Main 不得逐歌曲写缓存目录或 prepare 占位 URI；LOCAL 的 `content://` / file URI 必须绕过缓存。已缓存回退只允许当前音质下经 Media3 Span 实时复核为完整覆盖的 `READY` 项，未知长度、存在缺口或仅有部分字节的记录不得进入回退列表。目录索引继续写入 `pm_local_music.db` 的 `cached_playback_records`，兼容列 `play_url` 不得恢复运行时读写；清除歌曲缓存必须通过该门面同步清理物理缓存和目录。`Media3CacheStore` 任一时刻只持有一个 `SimpleCache`，播放器释放后关闭并清空引用，后续播放器按最新缓存上限惰性重建；清理 action 不得 release 活跃 cache。
