...
- 游客只显示普通用户档位：WY `128k / 标准 / 较高 / 极高`，其中前两项可用，后两项灰色并标记蓝色“需登录”；KG `128 / 320 / 无损（high）`，其中仅 128 可用，后两项灰色并标记“需登录”。点击“需登录”音质关闭选择器并进入 PisaMusic 登录页，不能选中或继续取链。
...
- [ ] 游客的受限普通音质为灰色，右侧显示蓝色“需登录”，点击后关闭弹窗并进入 PisaMusic 登录页。
