...
/**
 * 「我的收藏」详情：复用歌单详情布局，数据来自 [LoveManager]，封面为与「我的」页收藏入口一致的本地图。
 */
@AndroidEntryPoint
class LovedSongsPlaylistActivity : BaseDownloadActivity() {
    ...
    private val listAdapter = LovedSongsPlaylistAdapter(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                val color = ContextCompat.getColor(this@LovedSongsPlaylistActivity, iconTint)
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                loveManager.loveListFlow.collect { songs ->
                    listAdapter.setSongs(this@LovedSongsPlaylistActivity, songs)
                }
            }
        }
    }
...
private class LovedSongsPlaylistAdapter(
    ...
    private var coverDrawableRes: Int = R.drawable.my_favorites_cover_peach
    ...
    fun bindStaticHeader(title: String, desc: String, coverDrawableRes: Int) {
        headerTitle = title
        headerDesc = desc
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            VT_HEADER -> FavHeaderVh(ItemPlaylistDetailHeaderBinding.inflate(inflater, parent, false))
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            ...
            is StatusVh -> holder.bind(holder.itemView.context.getString(R.string.loved_songs_empty_hint))
    ...
    private class FavHeaderVh(
        ...
        fun bind(
            ...
            binding.coverImageView.load(coverDrawableRes) {
                crossfade(true)
                placeholder(coverDrawableRes)
                error(coverDrawableRes)
            }
            binding.headerBgImageView.load(coverDrawableRes) {
                crossfade(true)
                placeholder(coverDrawableRes)
                error(coverDrawableRes)
            }
            binding.headerBlurImageView.load(coverDrawableRes) {
                crossfade(true)
                placeholder(coverDrawableRes)
                error(coverDrawableRes)
            }
...
private val lovedTrailingParenInTitle = Regex("\\([^)]*\\)\\s*$")

private fun splitLovedTrailingParenTitle(title: String): Pair<String, String?> {
    val t = title.trimEnd()
    val m = lovedTrailingParenInTitle.find(t) ?: return t to null
    val main = t.substring(0, m.range.first).trimEnd()
    return if (main.isEmpty()) t to null else main to m.value.trim()
}

private fun buildLovedSongNameLine(context: Context, indexOneBased: Int, title: String): CharSequence {
    ...
    val (main, paren) = splitLovedTrailingParenTitle(title)
    b.append(main)
    if (paren != null) {
        val pStart = b.length
        b.append(paren)
