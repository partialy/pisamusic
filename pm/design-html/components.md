...
| 无图标二级设置页 | 播放、下载、歌词等分组设置页 | `pm/app/src/main/res/layout/activity_sub_settings.xml`、`item_sub_settings_*.xml` | `SubSettingsActivity`、`SubSettingsAdapter`、`SubSettingsItem` | 子页只声明标题、分组和 Option / Navigation / Switch；统一使用无图标行，不复制页面外壳。 |
