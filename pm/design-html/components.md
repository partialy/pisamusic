...
| 无图标二级设置页 | 播放、下载、歌词等分组设置页 | `pm/app/src/main/res/layout/activity_sub_settings.xml`、`item_sub_settings_*.xml` | `SubSettingsActivity`、`SubSettingsAdapter`、`SubSettingsItem`、`DownloadSettingsActivity` | 子页只声明标题、分组和 Option / Navigation / Switch；`summary` 可选，有说明时使用较小、较淡的第二行，无说明时保持原单行布局。 |
