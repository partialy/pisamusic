...
- 账号接口：`/api/auth/email-code` 发送邮箱验证码，`/api/auth/register` 注册，`/api/auth/login/password` 支持用户名/邮箱+密码登录，`/api/auth/login/code` 仅支持邮箱验证码登录，`/api/auth/refresh` 刷新 7 天 token，`/api/auth/me` 获取当前账号。
