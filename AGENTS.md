...
- 账号资料由 `/api/auth/profile*` 提供服务，支持修改昵称、头像 key 和经新邮箱验证码确认后的邮箱；公开用户字段包含 `id`、`username`、`email`、`avatarKey`、`avatarUrl`、`createdAt`。默认头像固定使用 `server/static/account-avatars/default.jpg`；自定义头像通过 `/api/auth/avatar/upload-token` 获取七牛 token 后由客户端直传到 `QINIU_PUBLIC_IMAGE_BUCKET` 公开图片空间，用户表 `avatar_key` 记录七牛对象 key，`avatarUrl` 直接返回公开直链；旧的多张内置头像自选功能已废弃，不要恢复。
...
- 分享记录后台管理接口挂载在 `/api/admin/shares`，支持按 `type=song|playlist`、分享人关键词和 `valid=true|false|all` 分页筛选，并展示访问次数和最近更新时间；单条分享只能通过 `PATCH /api/admin/shares/:uuid/invalid` 标记失效，必须保留 `share_records` 历史记录，不删除 `raw_json`，也不要改变 `GET /api/shares/public/:uuid` 的公开读取契约。
...
- 播放故障上报由 Android `playback_fault_logs` 本地表记录 KG / WY / KW 取链失败、空地址、非法地址和在线歌曲播放器失败，最多保留 300 条；上传只选择 `is_upload=0` 的日志并在服务端确认后按日志 UUID 精确标记。上报接口为加密 `POST /api/fault-reports`，允许匿名提交，有效账号 token 仅绑定用户 ID；服务端使用 `fault_reports` / `fault_report_logs` 按客户端日志 UUID 去重。后台独立挂载 `/api/admin/fault-reports`，支持筛选、详情、`pending` / `processed` 状态流转和级联删除批次。
