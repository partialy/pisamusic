...
- 实时通信模块：`server/src/realtime/` 使用 Socket.IO 绑定 HTTP server；一起听接口挂载 `/api/listen-together`，房间状态由 `server/src/db/listenTogetherStore.ts` 内存维护，人数上限读取动态配置 `listen_together_max_people`，仅 `/api/listen-together/config` 默认明文开放。一起听房间队列不存服务端，`listen:queue` / `QUEUE_EVENT` 只做房间成员校验和转发。
