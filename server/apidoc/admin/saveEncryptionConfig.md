...
| 字段名 | 类型 | 必填 | 说明与限制 |
| :--- | :--- | :--- | :--- |
| `plaintextPaths` | `string[]` | 是 | 白名单路径字符串数组（单条最多 256 字符，上限 200 条；用户配置只支持末尾 `*` 通配符） |
...
服务端会剥离请求中已有的强制路径后再统一合并保存，因此将读取结果原样提交不会失败。当前强制项包括 `/api/cloud-music/tracks/*/cover`：该精确封面路由使用单路径段通配，仅由服务端维护，不能借此配置任意中间通配。
