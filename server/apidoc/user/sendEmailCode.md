...
向指定邮箱发送验证码，用于账号注册、验证码快捷登录或重置密码。
...
| `purpose` | `string` | 是 | 验证码用途，可选值：<br>• `register`：注册验证（已注册邮箱会报错）<br>• `login`：登录验证（未注册邮箱会报错）<br>• `reset_password`：重置密码（未注册邮箱会报错） |
...
- `404 该邮箱尚未注册`：`purpose` 为 `login` 或 `reset_password` 且邮箱不存在时返回。
- `400 邮箱格式不正确`：邮箱校验失败。
